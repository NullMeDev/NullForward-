...
@app.route('/')
def home():
    return "XHOST Bot - Script Hosting Service"
...
def run_flask():
    ...
    try:
        ...
        logger.error(f"❌ Flask error (port {port}): {e}. Bot will continue without keep-alive.")
...
if not TOKEN:
    raise ValueError("❌ TELEGRAM_BOT_TOKEN not set. Create .env file with your bot token.")

OWNER_ID = int(os.getenv('OWNER_ID', '0'))
if OWNER_ID == 0:
    raise ValueError("❌ OWNER_ID not set. Add it to .env file.")

ADMIN_ID = int(os.getenv('ADMIN_ID', str(OWNER_ID)))
YOUR_USERNAME = os.getenv('BOT_USERNAME', 'your_bot').lstrip('@')
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL', f'https://t.me/{YOUR_USERNAME}')
...
# File upload limits - UNLIMITED for all users
FILE_UPLOAD_LIMIT = float('inf')
...
def get_user_file_limit(user_id):
    """All users have unlimited file uploads"""
    return float('inf')
...
def create_subscription_menu():
    # Subscriptions removed - not needed
...
# Subscriptions removed - all users have unlimited files
...
# --- Subscription callbacks removed - all users have unlimited files ---
